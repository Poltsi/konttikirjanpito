...
class DB {
	...
	private $db_stmt = [
		...
		'get_cylinders_by_user'            => ['sql' => 'SELECT c.cylinder_id, c.type_id, c.name, c.identifier, c.inspection_date, c.added, ct.label, ct.name AS type_name, ct.pressure, ct.size FROM cylinders c, cylinder_types ct WHERE ct.type_id = c.type_id AND c.user_id = $1 ORDER BY c.cylinder_id ASC'],
		'get_cylinders'                    => ['sql' => 'SELECT c.user_id, u.login, u.level, cylinder_id, c.type_id, c.name, c.identifier, c.inspection_date, c.added, ct.label, ct.name AS type_name, ct.pressure, ct.size FROM cylinders c, cylinder_types ct, users u WHERE u.uid = c.user_id AND ct.type_id = c.type_id ORDER BY c.cylinder_id ASC'],
